...
class mydsp : public dsp_2 {
 ...
 public:
	...
	virtual void instanceConstants(int sample_rate) {
		...
		fConst0 = 2.2e+02f / std::min<float>(1.92e+05f, std::max<float>(1.0f, float(fSampleRate)));
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			output0[i0] = FAUSTFLOAT(mult(ftbl0mydspSIG0[std::max<int>(0, std::min<int>(int(65536.0f * fRec1[0]), 65535))], 0.5f));
			iVec1[1] = iVec1[0];
			fRec1[1] = fRec1[0];
		}
	}
